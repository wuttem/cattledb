...
class StorageTest(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        db.insert("sensor2", "ph", d3)

        r = db.get_single_timeseries("sensor1", "act", 0, 500*600-1)
        a = list(r.all())
        ...
        r = db.get_single_timeseries("sensor1", "ph", 0, 500*600-1)
        a = list(r.all())
        d = list(r.aggregation("daily", "mean"))
        self.assertEqual(len(a), 500)
        self.assertEqual(len(d), 4)
        ...
        r = db.get_single_timeseries("sensor1", "temp", 24 * 60 * 60, 24 * 60 * 60 + 500*600)
        a = list(r.all())
        d = list(r.aggregation("daily", "mean"))
        ...
        s = db.get_last_values("sensor1", ["temp", "ph"], count=200)
        temp = s[0]
